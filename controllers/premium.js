...
const reportCard = async (req, res) => {
  ...
  try {
    ...
    const categoryArr = Object.values(user.toObject()).filter((elm) => elm !== 0);

    console.log(user);
    console.log('**********************************************************************');

    // Fetch the user's expenses, sorted by category in descending order
    const expenses = await Expense.find({ userId }).sort({ category: 'desc' });
    ...
    expenses.forEach((elm, ind) => {
      console.log('=============================================');
      if (ind !== expenses.length - 1 && elm.category !== expenses[ind + 1].category) {
        arr.push(elm);
        arr.push({ [elm.category]: categoryArr[i++] });
      } else {
        arr.push(elm);
